...
function CustomerEditProfile() {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((response) => {
        setInputData({
          ...response.data,
          currentPassword: "",  // Hide current password in the input field
          newPassword: ""       // New password is empty initially
        });
        setLoading(false);
      })
      .catch((error) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Only send newPassword if it's not empty
      if (inputData.newPassword.trim() !== "") {
        updatedData.password = inputData.newPassword;
      }

      await axios.post(`http://localhost:8787/customerside/updatecustomer`, updatedData, {
        headers: {
          ...
          <div className="form-group">
            ...
            <div className="input-group">
              <input
                ...
                onChange={(e) =>
                  setInputData({ ...inputData, currentPassword: e.target.value })
                }
                required
              />
              <button
